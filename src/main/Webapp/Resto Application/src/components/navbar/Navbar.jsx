...
const Navbar = () => {
  ...
  return (
    <nav className="bg-gray-200 shadow-md fixed w-full z-50">
      <div className="max-w-7xl mx-auto px-1 sm:px-4 lg:px-6 py-2 flex justify-between items-center">
        {/* Logo */}
        <Link to={user ? "/member-home" : "/"} className="text-red-600 font-bold text-2xl flex items-center flex-shrink-0">
        ...
        {!user && (
          <div className="hidden md:flex flex-1 justify-center min-w-0">
            <ul className="flex flex-row flex-wrap items-center justify-center gap-1 md:gap-2 lg:gap-3 xl:gap-4 text-gray-700 text-sm font-medium w-full">
        ...
        {/* Right Section */}
        <div className="hidden md:flex items-center gap-x-1 sm:gap-x-2 flex-shrink-0">
          {user ? (
            <MemberAvatarDropdown />
          ) : (
            <div className="flex items-center gap-x-1 sm:gap-x-2">
              <button className="border border-black px-4 sm:px-6 py-2 rounded-md hover:text-blue-700 text-sm transition-colors duration-150" onClick={() => navigate('/user-auth')}>
                Log in
              </button>
              <button className="border border-black px-3 sm:px-4 py-2 rounded-md hover:text-blue-700 text-sm flex items-center transition-colors duration-150" onClick={() => navigate('/member-auth')}>
                Join as Member
              </button>
            </div>
          )}
      ...
      {menuOpen && (
        <div className="md:hidden bg-white px-4 py-4 shadow-lg space-y-4 w-full">
          ...
          <div className="pt-4 border flex flex-col gap-2">
            <button className="w-full border border-black py-2 rounded-md text-sm hover:text-blue-700 transition-colors duration-150" onClick={() => { setMenuOpen(false); navigate('/user-auth'); }}>
              Log in
            </button>
            <button className="w-full border border-black py-2 rounded-md text-sm hover:text-blue-700 transition-colors duration-150" onClick={() => { setMenuOpen(false); navigate('/member-auth'); }}>
              Join as Member
            </button>
          </div>
        </div>
